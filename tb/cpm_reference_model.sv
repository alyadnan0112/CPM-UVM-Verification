class cpm_reference_model extends uvm_component;
        ...
        case (reg_txn.addr)
            ...
            begin
                ...
                if (CTRL_SOFT_RST) begin
                    ...
                    `uvm_info(get_type_name(),
                        $sformatf("CTRL Updated : ENABLE=%0b SOFT_RST=%0b",
                                CTRL_ENABLE,
                                CTRL_SOFT_RST),
                        UVM_LOW)
                end
            ...
            begin
                ...
                `uvm_info(get_type_name(),
                    $sformatf("PARAMS Updated : MASK=0x%0h ADD_CONST=0x%0h",
                            MASK,
                            ADD_CONST),
                    UVM_LOW)
            end
            ...
            begin
                ...
                `uvm_info(get_type_name(),
                    $sformatf("DROP_CFG Updated : DROP_EN=%0b DROP_OPCODE=%0h",
                            DROP_EN,
                            DROP_OPCODE),
                    UVM_LOW)
            end
            ...
            begin
                `uvm_warning(get_type_name(),
                    $sformatf("Unknown Register Address : 0x%0h",
                            reg_txn.addr))
            end
    ...
    function void write_packet(input cpm_packet pkt);
        ...
        if (!CTRL_ENABLE) begin  // CPM disabled
            ...
            `uvm_info(get_type_name(),
                "Packet ignored : CPM is disabled",
                UVM_LOW)
        ...
        if (DROP_EN && (pkt.opcode == DROP_OPCODE)) begin
            ...
            `uvm_info(get_type_name(),
                $sformatf("Packet Dropped : Opcode = %0h", pkt.opcode),
                UVM_LOW)
        ...
        case (MODE)
            ...
            begin
                ...
                `uvm_info(get_type_name(),
                    $sformatf("XOR Mode : %0h ^ %0h = %0h",
                            pkt.payload,
                            MASK,
                            expected_pkt.payload),
                    UVM_LOW)
            end
            ...
            2'b11:   // ROT Mode
            begin
                expected_pkt.payload = {pkt.payload[14:0], pkt.payload[15]};

                `uvm_info(get_type_name(),
                    $sformatf("ROT Mode : %0h -> %0h",
                            pkt.payload,
                            expected_pkt.payload),
                    UVM_LOW)
            end
    ...
    function void report_phase(uvm_phase phase);
        ...
        `uvm_info(get_type_name(),
        $sformatf("COUNT_IN=%0d COUNT_OUT=%0d DROPPED_COUNT=%0d",
                  COUNT_IN,
                  COUNT_OUT,
                  DROPPED_COUNT),
        UVM_LOW)
